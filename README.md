...
| 🧠 CLIP pre-analysis + rules | CLIP рахує візуальні сигнали (white bg / text overlay / collage / lifestyle / single product), OCR + правила блокують “ideal main” для фото з текстом |
| 🗂️ Ollama ранжування набору | Ollama отримує CLIP/OCR/rule сигнали та визначає **фінальний порядок** відображення фото |
...
|---|---|---|
| **1** | `main` | Ідеальне головне фото: чистий білий/світлий фон, один товар, **без тексту/плашок/бейджів/водяних знаків** |
...
#### 7. Ранжування набору фото через Ollama (нова функція)

Починаючи з цієї версії, Ollama **аналізує всі фотографії одного товару разом** — тобто бачить всю папку одночасно — і самостійно вирішує, який порядок відображення буде найкращим.

**Як це працює:**

1. Кожне фото аналізується окремо за допомогою OpenCV (колір фону) і Ollama (категорія).
2. Після цього всі зображення папки надсилаються **одним запитом** до Ollama.
3. Модель отримує до **20 зображень** одночасно і повертає список номерів у бажаному порядку.
4. Програма застосовує цей порядок як кінцевий і перейменовує файли відповідно.
5. Якщо Ollama не змогла ранжувати (помилка з'єднання, некоректна відповідь, забагато фото) — автоматично використовується стандартне сортування за категоріями.

**Вимоги:**
- Vision-модель, що підтримує кілька зображень одночасно: `llava`, `bakllava` або аналогічна.
- Кількість фото у папці: від 2 до 20. Якщо фото більше 20 — застосовується стандартне сортування.

**Важливо про "ideal main":**
- OCR або CLIP-сигнал `text_overlay` блокує статус “ідеальне головне фото”.
- Таке фото може бути лише “альтернативним головним” (потрібна обробка/видалення тексту).

**Журнал роботи** покаже:
- `Ollama: ранжування N фото разом…` — під час відправки запиту
- `✅ Порядок фото визначено Ollama (ранжування набору)` — якщо ранжування пройшло успішно
- У разі відмови — попередження в логах і автоматичний fallback без зупинки обробки
...
3. **Оберіть метод аналізу**:
   ...
   - *Ollama* — локальні моделі (llava, bakllava), не потребує API ключа, працює офлайн; **тепер також ранжує всі фото товару разом** (до 20 шт.) для кращого порядку
